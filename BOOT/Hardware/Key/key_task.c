...
static const MfKeyItemCfg_T S_tKeyMwKeyCfg[] =
{
	...
	#if(boardLIGHT_EN)
	[keyLIGHT] = {KTE_LIGHT_SHORT,  KTE_LIGHT_LONG,  false,  false},
	#endif  //boardLIGHT_EN

	#if(boardUSB_EN)
	[keyUSB]   = {KTE_USB_SHORT,    KTE_USB_LONG,    false,  false},
	#endif  //boardUSB_EN
 ...
 * 说明(备注)  : BOOT特有: GPIO初始化未完成或处于升级任务期间暂停按键扫描
 * 传入参数    : pvParameters
 * 返回值      : none
 ************************************************************************************************************************/
void vKey_Task(void *pvParameters)
{
	...
	#endif  //boardUSE_OS
	{
		//GPIO初始化未完成 / BOOT特有:升级任务期间不处理按键 (长按开机锁定)
...
/***********************************************************************************************************************
 * 函数功能    : 工厂模式组合按键检测 (Power + USB 按下, 其他按键未按下)
 * 传入参数    : none
 ...
 * 说明(备注)  : BOOT原逻辑仅判断息屏状态(无工作状态限制), 行为保持一致
