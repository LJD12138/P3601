...
static void gpio_config(void)
{
    ...
    rcu_periph_clock_enable(adcUSB_A_CURR_RCU);
	gpio_init(adcUSB_A_CURR_PORT, GPIO_MODE_AIN, GPIO_OSPEED_10MHZ, adcUSB_A_CURR_PIN);
	
	rcu_periph_clock_enable(adcUSB_A_VOLT_RCU);
	gpio_init(adcUSB_A_VOLT_PORT, GPIO_MODE_AIN, GPIO_OSPEED_10MHZ, adcUSB_A_VOLT_PIN);
	
	// rcu_periph_clock_enable(adcKEY_POWER_RCU);
	// gpio_init(adcKEY_POWER_PORT, GPIO_MODE_AIN, GPIO_OSPEED_10MHZ, adcKEY_POWER_PIN);

    rcu_periph_clock_enable(adcFAN_VOLT_RCU);
	gpio_init(adcFAN_VOLT_PORT, GPIO_MODE_AIN, GPIO_OSPEED_10MHZ, adcFAN_VOLT_PIN);
}
...
static void adc_config(void)
{
	...
	adc_regular_channel_config(ADCX, 4, adcUSB_VOLT_CH, ADC_SAMPLETIME_239POINT5);
	adc_regular_channel_config(ADCX, 5, adcUSB_A_CURR_CH, ADC_SAMPLETIME_239POINT5);
    adc_regular_channel_config(ADCX, 6, adcUSB_A_VOLT_CH, ADC_SAMPLETIME_239POINT5);
	// adc_regular_channel_config(ADCX, 7, adcKEY_POWER_CH, ADC_SAMPLETIME_239POINT5);
    adc_regular_channel_config(ADCX, 7, adcFAN_VOLT_CH, ADC_SAMPLETIME_239POINT5);
