...
//****************************************************函数声明****************************************************//
static bool b_modbus_jump_step(ModbusProtoRx1_t* proto, ModbusRxStep1_E step);
static s8 c_check_cmd_exist(u8 cmd);
static s8 c_proto_decrypt(ModbusProtoRx1_t* proto);
...
******************************************************************************************************************/
s8 cModbus1_RecProtoInit(ModbusProtoRx1_t** proto, u16 buff_len, u8 dev_addr, u16 cycle_time)
{
	s8 result = 1;
...
//	sMyPrint("Free Heap: %u\n", xPortGetFreeHeapSize());
	...
	// 动态分配内存
    size_t total_size = sizeof(ModbusProtoRx1_t) + buff_len;
	#if(boardUSE_OS)
	*proto = (ModbusProtoRx1_t*)pvPortMalloc(total_size);
	(*proto)->ucpFrameData = (u8*)pvPortMalloc(buff_len);
	#else
	*proto = (ModbusProtoRx1_t*)malloc(total_size);
	...
	if(*proto != NULL && (*proto)->ucpFrameData != NULL)
	{
		...
		b_modbus_jump_step(*proto, MRS1_ADDR);
	}
	else 
	{
		#if(boardUSE_OS)
...
-----返回值		小于0:操作失败   等于0:没操作    大于0:操作成功
******************************************************************************************************************/
s8 cModbus1_TransProtoInit(ModbusProtoTx1_t** proto, u16 buff_len, u8 dev_addr)
{
	s8 result = 1;
	...
	#endif
	// 动态分配内存
	size_t total_size = sizeof(ModbusProtoTx1_t) + buff_len;
	#if(boardUSE_OS)
    *proto = (ModbusProtoTx1_t*)pvPortMalloc(total_size);
	#else
    *proto = (ModbusProtoTx1_t*)malloc(total_size);
	#endif
...
-----返回值		小于0:操作失败   等于0:没操作    大于0:操作成功
******************************************************************************************************************/
s8 cModbus1_ProtoCreate(ModbusProtoTx1_t* proto, u8 cmd, u16 reg_addr, u8* data, u16 len)
{
	s8 result = 1;
	u8 uc_char_len = 0;
	...
	switch(cmd)
	{
		case modbusWRITE_MULTI_REG:
		{
			...
			proto->ucaFrameData[0] = proto->ucAddr;
			proto->ucaFrameData[1] = cmd;
			proto->ucaFrameData[2] = reg_addr & 0x00ff;
			proto->ucaFrameData[3] = reg_addr >> 8;
			proto->ucaFrameData[4] = len & 0x00ff;
			proto->ucaFrameData[5] = len >> 8;  
			// proto->ucaFrameData[6] = uc_char_len;
			
			memcpy(&proto->ucaFrameData[6], (u8*)data, uc_char_len);
			
			us_total_len = 6 + uc_char_len;
...
-----返回值      小于0:操作失败   等于0:没操作    大于0:操作成功
******************************************************************************************************************/
s8 cModbus1_ProtoCheck(ModbusProtoRx1_t* proto)
{
	s8 c_result = 0;
	u8 temp[10] = {0};
	vu16 delay_cnt = 512;
		...
		case MRS1_ADDR:  //地址
		{
			...
			while(1)
			{
				...
				if(temp[0] == proto->ucAddr && c_result != 0)
				{
					...
					if(c_result < 0)
					{
						b_modbus_jump_step(proto, MRS1_ERR);
						return 0;
					}
					...
					b_modbus_jump_step(proto, MRS1_LEN);
					break;
				}
				else 
				{
			...
			//引导码可能丢失
			if(proto->eStep == MRS1_ADDR)
				return -4;
		}

		case MRS1_LEN: //等待接收长度
		{
			if(lwrb_get_full(&proto->tRxBuff) >= proto->ucWaitRecLen)
			{
				//取出长度
				if(proto->ucCmd == modbusWRITE_MULTI_REG)
				{
					...
					memcpy((u8*)&proto->usRegAddr, &temp[0], 2);
					memcpy((u8*)&proto->usRegSize, &temp[2], 2);
					
					memcpy(&proto->ucpFrameData[2], temp, 4);
					
					proto->ucCharLen = proto->usRegSize * 2;
				...
				b_modbus_jump_step(proto, MRS1_END);
			}
			else 
				break;
		}
...
-----返回值      小于0:操作失败   等于0:没操作    大于0:操作成功
******************************************************************************************************************/
s8 cModbus1_StepWaitOutTime(ModbusProtoRx1_t* proto)
{
	if(proto == NULL)
		return -1;
...
-----返回值      小于0:操作失败   等于0:没操作    大于0:操作成功
******************************************************************************************************************/
s8 cModbus1_ResetRxBuff(ModbusProtoRx1_t* proto)
{
	if(proto == NULL)
		return -1;
...
-----返回值      小于0:操作失败   等于0:没操作    大于0:操作成功
******************************************************************************************************************/
s8 cModbus1_ResetTx(ModbusProtoTx1_t* proto, u16 len)
{
	if(proto == NULL)
		return -1;
...
-----返回值      小于0:操作失败   等于0:没操作    大于0:操作成功
******************************************************************************************************************/
void vModbus1_RecEnd(ModbusProtoRx1_t* proto)
{
	if(proto == NULL)
		return;
...
static s8 c_proto_decrypt(ModbusProtoRx1_t* proto)
{
	...
	switch(proto->ucCmd)
	{
		case modbusWRITE_MULTI_REG:
		{
			us_total_len = 6 + proto->ucCharLen;
			proto->ucValidLen = 0;
			
			lwrb_read(&proto->tRxBuff, &proto->ucpFrameData[6], proto->ucCharLen);
			proto->ucpValidData = &proto->ucpFrameData[6];
